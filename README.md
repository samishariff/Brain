...
**Brain for Windows preview 1.0.3.15 is available** from the [Brain website](https://samishariff.github.io/Brain/). Read the [preview notes](https://samishariff.github.io/Brain/release-notes.html) before installing. Physical microphones, live calls, device changes and sleep/wake behavior remain unverified for this preview.
...
The Windows download choices on the homepage include **Brain Meeting Memory**, published by **TheExalted**. Microsoft Store manages updates for that edition. Its library is separate from the website edition; see the [Windows help](help.html) for complete backup transfer and reset/uninstall guidance. The editions may receive updates at different times.

The homepage uses the exact static Store listing URL and remains usable without JavaScript. The optional script is idempotent. To withdraw the homepage option, set `data-approved="false"`, add `hidden` to `windows-store-option`, and remove its anchor's `href`; changing approval alone does not hide an already visible static link.

Before publishing an activation, verify the approved listing and ordinary Store acquisition for product `9NTQ8V2MMQP1`. The reserved ID and anonymous product metadata alone are not installed-app evidence. Reconcile the Windows release pipeline baseline after this website change merges so later app updates preserve the link and help text.
