...
In `index.html`, `windows-store-option` starts with an empty `data-product-id`
and `data-approved="false"`. Keep both defaults until Brain has an approved,
public Microsoft Store listing. The option stays hidden and has no link while
disabled; the existing Windows installer and Mac downloads remain available.
...
Then set `data-product-id` to that Store product ID and `data-approved="true"`.
...
activating the option. No Store product ID or active Store link is included here.
